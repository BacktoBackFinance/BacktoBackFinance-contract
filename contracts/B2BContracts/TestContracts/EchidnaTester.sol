...
contract EchidnaTester {
    ...
    uint private MCR;
    uint private CCR;
    uint private BUSDC_GAS_COMPENSATION;
    ...
    constructor() public payable {
        ...
        gasPool = new GasPool();
        busdcToken = new BUSDCToken(address(troveManager), address(stabilityPool), address(borrowerOperations));
        ...
        for (uint i = 0; i < NUMBER_OF_ACTORS; i++) {
            echidnaProxies[i] = new EchidnaProxy(troveManager, borrowerOperations, stabilityPool, busdcToken);
        ...
        BUSDC_GAS_COMPENSATION = borrowerOperations.BUSDC_GAS_COMPENSATION();
        require(MCR > 0);
        require(CCR > 0);
    ...
    function getAdjustedETH(uint actorBalance, uint _ETH, uint ratio) internal view returns (uint) {
        ...
        require(price > 0);
        uint minETH = ratio.mul(BUSDC_GAS_COMPENSATION).div(price);
        require(actorBalance > minETH);
        uint ETH = minETH + (_ETH % (actorBalance - minETH));
        return ETH;
    }

    function getAdjustedBUSDC(uint ETH, uint _BUSDCamount, uint ratio) internal view returns (uint) {
        uint price = priceFeedTestnet.getPrice();
        uint BUSDCAmount = _BUSDCamount;
        uint compositeDebt = BUSDCAmount.add(BUSDC_GAS_COMPENSATION);
        uint ICR = LiquityMath._computeCR(ETH, compositeDebt, price);
        if (ICR < ratio) {
            compositeDebt = ETH.mul(price).div(ratio);
            BUSDCAmount = compositeDebt.sub(BUSDC_GAS_COMPENSATION);
        }
        return BUSDCAmount;
    }

    function openTroveExt(uint _i, uint _ETH, uint _BUSDCamount) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        ...
        uint BUSDCAmount = getAdjustedBUSDC(ETH, _BUSDCamount, CCR);

        //console.log('ETH', ETH);
        //console.log('BUSDCAmount', BUSDCAmount);

        echidnaProxy.openTrovePrx(ETH, BUSDCAmount, address(0), address(0), 0);
    ...
    function openTroveRawExt(
        ...
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].openTrovePrx(_ETH, _BUSDCamount, _upperHint, _lowerHint, _maxFee);
    }
    ...
    function adjustTroveExt(
        ...
        if (_isDebtIncrease) {
            ...
            debtChange = getAdjustedBUSDC(ETH, uint(_debtChange), MCR);
        }
    ...
    function echidna_trove_properties() public view returns (bool) {
        ...
        while (currentTrove != address(0)) {
            ...
            // Minimum debt (gas compensation)
            if (troveManager.getTroveDebt(currentTrove) < BUSDC_GAS_COMPENSATION) {
                return false;
            }
            // Uncomment to check that the condition is meaningful
            //else return false;
    ...
    function echidna_ETH_balances() public view returns (bool) {
        ...
        if (address(busdcToken).balance > 0) {
            return false;
        }
    ...
    // Total BUSDC matches
    function echidna_BUSDC_global_balances() public view returns (bool) {
        uint totalSupply = busdcToken.totalSupply();
        uint gasPoolBalance = busdcToken.balanceOf(address(gasPool));

        uint activePoolBalance = activePool.getBUSDCDebt();
        uint defaultPoolBalance = defaultPool.getBUSDCDebt();
        if (totalSupply != activePoolBalance + defaultPoolBalance) {
            return false;
        }

        uint stabilityPoolBalance = stabilityPool.getTotalBUSDCDeposits();
        address currentTrove = sortedTroves.getFirst();
        uint trovesBalance;
        while (currentTrove != address(0)) {
            trovesBalance += busdcToken.balanceOf(address(currentTrove));
            currentTrove = sortedTroves.getNext(currentTrove);
        }
