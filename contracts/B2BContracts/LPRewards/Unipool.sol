...
/*
 * On deployment a new Uniswap pool will be created for the pair BUSDC/ETH and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of B2B token,
 ...
 * in order to avoid getting B2B tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either B2B token contract is deployed, and therefore B2B tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
        ...
        onlyOwner
    {
        ...
        _notifyRewardAmount(b2bToken.getLpRewardsEntitlement(), _duration);
    ...
    function claimReward() public override {
        ...
        b2bToken.transfer(msg.sender, reward);
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == b2bToken.balanceOf(address(this)));
