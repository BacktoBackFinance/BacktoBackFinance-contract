...
async function main() {
  ...
  const B2BContracts = await dh.deployB2BContractsHardhat(
      ARBITRARY_ADDRESS,
      ARBITRARY_ADDRESS,
      ARBITRARY_ADDRESS
    )
 ...
 const { troveManager, borrowerOperations, hintHelpers, sortedTroves, priceFeedTestnet } = coreContracts
  ...
  const BUSDCAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 BUSDC
  ...
  const liquidationReserve = await troveManager.BUSDC_GAS_COMPENSATION()
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(BUSDCAmount)

  // Total debt of the new trove = BUSDC amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = BUSDCAmount.add(expectedFee).add(liquidationReserve)
  ...
  await borrowerOperations.openTrove(maxFee, BUSDCAmount, upperHint, lowerHint, { value: ETHColl })

  // --- adjust trove ---

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 ETH
  const BUSDCRepayment = toBN(toWei('230')) // borrower wants to repay 230 BUSDC

  // Get trove's current debt and coll
  const {0: debt, 1: coll} = await troveManager.getEntireDebtAndColl(borrower)

  const newDebt = debt.sub(BUSDCRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, BUSDCRepayment, false, upperHint, lowerHint, {value: collIncrease})


  // --- RedeemCollateral ---

  // Get the redemptions hints from the deployed HintHelpers contract
  const redemptionhint = await hintHelpers.getRedemptionHints(BUSDCAmount, price, 50)

  const {0: firstRedemptionHint, 1: partialRedemptionNewICR, 2: truncatedBUSDCAmount} = redemptionhint
  ...
  */
  ...
  /* Finally, perform the on-chain redemption, passing the truncated BUSDC amount, the correct hints, and the expected
  * ICR of the final partially redeemed trove in the sequence.
  */
  await troveManager.redeemCollateral(truncatedBUSDCAmount,
    firstRedemptionHint,
