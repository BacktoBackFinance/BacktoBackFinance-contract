...
class TestHelper {
  ...
  // Adds the gas compensation (50 BUSDC)
  ...
  /*
   * given the requested BUSDC amomunt in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, busdcAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(busdcAmount)
    const compositeDebt = await this.getCompositeDebt(contracts, busdcAmount)
    return compositeDebt.add(fee)
  }

  /*
   * given the desired total debt, returns the BUSDC amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveBUSDCAmount(contracts, totalDebt) {
  ...
  static async getAmountWithBorrowingFee(contracts, busdcAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(busdcAmount)
    return busdcAmount.add(fee)
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        ...
        const BUSDCAmount = redemptionTx.logs[i].args[0]
        const totalBUSDCRedeemed = redemptionTx.logs[i].args[1]
        const totalETHDrawn = redemptionTx.logs[i].args[2]
        const ETHFee = redemptionTx.logs[i].args[3]

        return [BUSDCAmount, totalBUSDCRedeemed, totalETHDrawn, ETHFee]
      }
    }
  ...
  static getEmittedLiquidationValues(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const busdcGasComp = liquidationTx.logs[i].args[3]

        return [liquidatedDebt, liquidatedColl, collGasComp, busdcGasComp]
      }
    }
  ...
  static getBUSDCFeeFromBUSDCBorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "BUSDCBorrowingFeePaid") {
        return (tx.logs[i].args[1]).toString()
      }
    }
    throw ("The transaction logs do not contain an BUSDCBorrowingFeePaid event")
  }
  ...
  static async getEntireCollAndDebt(contracts, account) {
    ...
    const pendingBUSDCDebtReward = await contracts.troveManager.getPendingBUSDCDebtReward(account)
    const entireColl = rawColl.add(pendingETHReward)
    const entireDebt = rawDebt.add(pendingBUSDCDebtReward)
  ...
  static async getCollAndDebtFromWithdrawBUSDC(contracts, account, amount) {
  ...
  static async getCollAndDebtFromRepayBUSDC(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)
  ...
  static async getCollAndDebtFromAdjustment(contracts, account, ETHChange, BUSDCChange) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)

    // const coll = (await contracts.troveManager.Troves(account))[1]
    // const debt = (await contracts.troveManager.Troves(account))[0]

    const fee = BUSDCChange.gt(this.toBN('0')) ? await contracts.troveManager.getBorrowingFee(BUSDCChange) : this.toBN('0')
    const newColl = entireColl.add(ETHChange)
    const newDebt = entireDebt.add(BUSDCChange).add(fee)
  ...
  static async openTrove_allAccounts(accounts, contracts, ETHAmount, BUSDCAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, BUSDCAmount)
  ...
  static async openTrove_allAccounts_randomETH(minETH, maxETH, accounts, contracts, BUSDCAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, BUSDCAmount)
  ...
  static async openTrove_allAccounts_randomETH_ProportionalBUSDC(minETH, maxETH, accounts, contracts, proportion) {
    const gasCostList = []

    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minETH, maxETH)
      const proportionalBUSDC = (web3.utils.toBN(proportion)).mul(web3.utils.toBN(randCollAmount))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalBUSDC)
  ...
  static async openTrove_allAccounts_randomETH_randomBUSDC(minETH, maxETH, accounts, contracts, minBUSDCProportion, maxBUSDCProportion, logging = false) {
    const gasCostList = []
    const price = await contracts.priceFeedTestnet.getPrice()
    ...
    for (const account of accounts) {
      ...
      const randBUSDCProportion = this.randAmountInWei(minBUSDCProportion, maxBUSDCProportion)
      const proportionalBUSDC = (web3.utils.toBN(randBUSDCProportion)).mul(web3.utils.toBN(randCollAmount).div(_1e18))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalBUSDC)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, randCollAmount, totalDebt)
      ...
      if (logging && tx.receipt.status) {
        i++
        const ICR = await contracts.troveManager.getCurrentICR(account, price)
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalBUSDC} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
  ...
  static async openTrove_allAccounts_randomBUSDC(minBUSDC, maxBUSDC, accounts, contracts, ETHAmount) {
    const gasCostList = []

    for (const account of accounts) {
      const randBUSDCAmount = this.randAmountInWei(minBUSDC, maxBUSDC)
  ...
  static async openTrove_allAccounts_decreasingBUSDCAmounts(accounts, contracts, ETHAmount, maxBUSDCAmount) {
    const gasCostList = []

    let i = 0
    for (const account of accounts) {
      const BUSDCAmount = (maxBUSDCAmount - i).toString()
      const BUSDCAmountWei = web3.utils.toWei(BUSDCAmount, 'ether')
  ...
  static async openTrove(contracts, {
    ...
    if (!extraBUSDCAmount) extraBUSDCAmount = this.toBN(0)
    else if (typeof extraBUSDCAmount == 'string') extraBUSDCAmount = this.toBN(extraBUSDCAmount)
    ...
    const busdcAmount = MIN_DEBT.add(extraBUSDCAmount)
    ...
    const backedAmount = extraParams.value
    const sender = extraParams.from
    await contracts.backedToken.approve(contracts.borrowerOperations.address, backedAmount, {from: sender})
    const tx = await contracts.borrowerOperations.openTrove(maxFeePercentage, busdcAmount, upperHint, lowerHint, backedAmount, {from: sender})
    ...
    return {
      ...
      totalDebt,
      ICR,
      collateral: backedAmount,
      tx
    }
  }

  static async withdrawBUSDC(contracts, {
    maxFeePercentage,
    busdcAmount,
    ICR,
    upperHint,
    ...
    assert(!(busdcAmount && ICR) && (busdcAmount || ICR), "Specify either busdc amount or target ICR, but not both")
    ...
    if (ICR) {
      ...
      busdcAmount = await this.getNetBorrowingAmount(contracts, increasedTotalDebt)
    } else {
      increasedTotalDebt = await this.getAmountWithBorrowingFee(contracts, busdcAmount)
    }

    await contracts.borrowerOperations.withdrawBUSDC(maxFeePercentage, busdcAmount, upperHint, lowerHint, extraParams)
  ...
  static async adjustTrove_allAccounts(accounts, contracts, ETHAmount, BUSDCAmount) {
    const gasCostList = []
    ...
    for (const account of accounts) {
      ...
      let BUSDCChangeBN = this.toBN(BUSDCAmount)

      const { newColl, newDebt } = await this.getCollAndDebtFromAdjustment(contracts, account, ETHChangeBN, BUSDCChangeBN)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const zero = this.toBN('0')

      let isDebtIncrease = BUSDCChangeBN.gt(zero)
      BUSDCChangeBN = BUSDCChangeBN.abs()

      // Add ETH to trove
      if (ETHChangeBN.gt(zero)) {
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, 0, BUSDCChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account, value: ETHChangeBN })
      // Withdraw ETH from trove
      } else if (ETHChangeBN.lt(zero)) {
        ETHChangeBN = ETHChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, ETHChangeBN, BUSDCChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account })
      }
  ...
  static async adjustTrove_allAccounts_randomAmount(accounts, contracts, ETHMin, ETHMax, BUSDCMin, BUSDCMax) {
    const gasCostList = []
    ...
    for (const account of accounts) {
      ...
      let BUSDCChangeBN = this.toBN(this.randAmountInWei(BUSDCMin, BUSDCMax))
      ...
      // Withdraw ETH from trove
        ...
        ETHChangeBN = ETHChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, ETHChangeBN, BUSDCChangeBN, isDebtIncrease, lowerHint,  upperHint,{ from: account })
      }

      const gas = this.gasUsed(tx)
      // console.log(`ETH change: ${ETHChangeBN},  BUSDCChange: ${BUSDCChangeBN}, gas: ${gas} `)
  ...
  static async withdrawBUSDC_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawBUSDC(contracts, account, amount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawBUSDC(this._100pct, amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawBUSDC_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randBUSDCAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawBUSDC(contracts, account, randBUSDCAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawBUSDC(this._100pct, randBUSDCAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repayBUSDC_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepayBUSDC(contracts, account, amount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repayBUSDC(amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repayBUSDC_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randBUSDCAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromRepayBUSDC(contracts, account, randBUSDCAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repayBUSDC(randBUSDCAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async redeemCollateral(redeemer, contracts, BUSDCAmount, gasPrice = 0, maxFee = this._100pct) {
    const price = await contracts.priceFeedTestnet.getPrice()
    const tx = await this.performRedemptionTx(redeemer, price, contracts, BUSDCAmount, maxFee, gasPrice)
    const gas = await this.gasUsed(tx)
    return gas
  }

  static async redeemCollateralAndGetTxObject(redeemer, contracts, BUSDCAmount, gasPrice, maxFee = this._100pct) {
  ...
  static async redeemCollateral_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const redeemer of accounts) {
      ...
      await this.performRedemptionTx(redeemer, price, contracts, randBUSDCAmount)
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async performRedemptionTx(redeemer, price, contracts, BUSDCAmount, maxFee = 0, gasPrice_toUse = 0) {
    const redemptionhint = await contracts.hintHelpers.getRedemptionHints(BUSDCAmount, price, gasPrice_toUse)
    ...
    const tx = await contracts.troveManager.redeemCollateral(BUSDCAmount,
      firstRedemptionHint,
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    const gasCostList = []
    for (const account of accounts) {
      const randomBUSDCAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.provideToSP(randomBUSDCAmount, this.ZERO_ADDRESS, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawFromSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      const randomBUSDCAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.withdrawFromSP(randomBUSDCAmount, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async getTimeFromSystemDeployment(b2bToken, web3, timePassedSinceDeployment) {
    const deploymentTime = await b2bToken.getDeploymentStartTime()
