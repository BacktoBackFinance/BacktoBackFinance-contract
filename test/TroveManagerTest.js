...
contract('TroveManager', async accounts => {
  ...
  const getOpenTroveTotalDebt = async (busdcAmount) => th.getOpenTroveTotalDebt(contracts, busdcAmount)
  ...
  const withdrawBUSDC = async (params) => th.withdrawBUSDC(contracts, params)
  ...
  beforeEach(async () => {
    ...
    await contracts.backedToken.mint(erin, dec(10000000000, 'ether'))
    await contracts.backedToken.mint(flyn, dec(10000000000, 'ether'))
    await contracts.backedToken.mint(graham, dec(10000000000, 'ether'))
    await contracts.backedToken.mint(harriet, dec(10000000000, 'ether'))
    await contracts.backedToken.mint(ida, dec(10000000000, 'ether'))

    await contracts.backedToken.mint(defaulter_1, dec(10000000000, 'ether'))
    await contracts.backedToken.mint(defaulter_2, dec(10000000000, 'ether'))
    await contracts.backedToken.mint(defaulter_3, dec(10000000000, 'ether'))
    await contracts.backedToken.mint(defaulter_4, dec(10000000000, 'ether'))
  ...
  it('liquidate(): closes a Trove that has ICR < MCR', async () => {
    ...
    // Alice increases debt to 180 BUSDC, lowering her ICR to 1.11
    const A_BUSDCWithdrawal = await getNetBorrowingAmount(dec(130, 18))

    const targetICR = toBN('1111111111111111111')
    await withdrawBUSDC({ ICR: targetICR, extraParams: { from: alice } })
    ...
    // price drops to 1ETH:100BUSDC, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool ETH and BUSDCDebt by correct amounts", async () => {
    // --- SETUP ---
    ...
    // check ActivePool ETH and BUSDC debt before
    const activePool_ETH_Before = (await activePool.getETH()).toString()
    const activePool_RawEther_Before = (await contracts.backedToken.balanceOf(activePool.address)).toString()
    const activePool_BUSDCDebt_Before = (await activePool.getBUSDCDebt()).toString()
    ...
    th.assertIsApproximatelyEqual(activePool_BUSDCDebt_Before, A_totalDebt.add(B_totalDebt))

    // price drops to 1ETH:100BUSDC, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his ether and BUSDC,
    leaving Alice’s ether and BUSDC debt in the ActivePool. */
    await troveManager.liquidate(bob, { from: owner });

    // check ActivePool ETH and BUSDC debt
    const activePool_ETH_After = (await activePool.getETH()).toString()
    const activePool_RawEther_After = (await contracts.backedToken.balanceOf(activePool.address)).toString()
    const activePool_BUSDCDebt_After = (await activePool.getBUSDCDebt()).toString()

    assert.equal(activePool_ETH_After, A_collateral)
    assert.equal(activePool_RawEther_After, A_collateral)
    th.assertIsApproximatelyEqual(activePool_BUSDCDebt_After, A_totalDebt)
  })

  it("liquidate(): increases DefaultPool ETH and BUSDC debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(21, 17)), extraParams: { from: bob } })
    ...
    // check DefaultPool ETH and BUSDC debt before
    const defaultPool_ETH_Before = (await defaultPool.getETH())
    const defaultPool_RawEther_Before = (await contracts.backedToken.balanceOf(defaultPool.address)).toString()
    const defaultPool_BUSDCDebt_Before = (await defaultPool.getBUSDCDebt()).toString()

    assert.equal(defaultPool_ETH_Before, '0')
    assert.equal(defaultPool_RawEther_Before, '0')
    assert.equal(defaultPool_BUSDCDebt_Before, '0')
    ...
    const defaultPool_RawEther_After = (await contracts.backedToken.balanceOf(defaultPool.address)).toString()
    const defaultPool_BUSDCDebt_After = (await defaultPool.getBUSDCDebt()).toString()
    ...
    th.assertIsApproximatelyEqual(defaultPool_BUSDCDebt_After, B_totalDebt)
  })
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // close Bob's Trove.  His ether*0.995 and BUSDC should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });
  ...
  it("liquidate(): updates the L_ETH and L_BUSDCDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    ...
    // price drops to 1ETH:100BUSDC, reducing Carols's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');
    ...
    // Carol's ether*0.995 and BUSDC should be added to the DefaultPool.
    const L_ETH_AfterCarolLiquidated = await troveManager.L_ETH()
    const L_BUSDCDebt_AfterCarolLiquidated = await troveManager.L_BUSDCDebt()

    const L_ETH_expected_1 = th.applyLiquidationFee(C_collateral).mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    const L_BUSDCDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    assert.isAtMost(th.getDifference(L_ETH_AfterCarolLiquidated, L_ETH_expected_1), 100)
    assert.isAtMost(th.getDifference(L_BUSDCDebt_AfterCarolLiquidated, L_BUSDCDebt_expected_1), 100)

    // Bob now withdraws BUSDC, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawBUSDC({ ICR: toBN(dec(111, 16)), extraParams: { from: bob } })

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    // price drops to 1ETH:50BUSDC, reducing Bob's ICR below MCR
   ...
   L_BUSDCDebt = (180 / 20) + (890 / 10) = 98 BUSDC */
    const L_ETH_AfterBobLiquidated = await troveManager.L_ETH()
    const L_BUSDCDebt_AfterBobLiquidated = await troveManager.L_BUSDCDebt()
    ...
    const L_BUSDCDebt_expected_2 = L_BUSDCDebt_expected_1.add(B_totalDebt.add(B_increasedTotalDebt).add(B_collateral.mul(L_BUSDCDebt_expected_1).div(mv._1e18BN)).mul(mv._1e18BN).div(A_collateral))
    assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2), 100)
    assert.isAtMost(th.getDifference(L_BUSDCDebt_AfterBobLiquidated, L_BUSDCDebt_expected_2), 100)
  })

  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    await openTrove({ ICR: toBN(dec(200, 18)), extraParams: { from: bob, value: dec(100, 'ether') } })

    // Alice creates a single trove with 0.7 ETH and a debt of 70 BUSDC, and provides 10 BUSDC to SP
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

    // Alice proves 10 BUSDC to SP
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides BUSDC to SP
    const spDeposit = toBN(dec(100, 24))
    await openTrove({ ICR: toBN(dec(4, 18)), extraBUSDCAmount: spDeposit, extraParams: { from: whale } })
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(3, 18)), extraBUSDCAmount: spDeposit, extraParams: { from: bob } })
    const { C_totalDebt, C_collateral } = await openTrove({ ICR: toBN(dec(218, 16)), extraBUSDCAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Bob sends tokens to Dennis, who has no trove
    await busdcToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides BUSDC to SP
    ...
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedBUSDCDeposit(dennis)).toString()
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedBUSDCDeposit(dennis)).toString()
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
    ...
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraBUSDCAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(218, 16)), extraBUSDCAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    //Bob provides BUSDC to SP
    ...
    const bob_Deposit_Before = (await stabilityPool.getCompoundedBUSDCDeposit(bob)).toString()
    ...
    const bob_Deposit_After = (await stabilityPool.getCompoundedBUSDCDeposit(bob)).toString()
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
    ...
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraBUSDCAmount: A_spDeposit, extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(218, 16)), extraBUSDCAmount: B_spDeposit, extraParams: { from: bob } })
    const { collateral: C_collateral, totalDebt: C_debt } = await openTrove({ ICR: toBN(dec(210, 16)), extraBUSDCAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Check Bob' SP deposit has absorbed Carol's debt, and he has received her liquidated ETH
    const bob_Deposit_Before = await stabilityPool.getCompoundedBUSDCDeposit(bob)
    ...
    /* Alice's BUSDC Loss = (300 / 400) * 200 = 150 BUSDC
       Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

       Bob's BUSDCLoss = (100 / 400) * 200 = 50 BUSDC
       Bob's ETH gain = (100 / 400) * 2*0.995 = 0.4975 ETH

     Check Bob' SP deposit has been reduced to 50 BUSDC, and his ETH gain has increased to 1.5 ETH. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedBUSDCDeposit(alice)).toString()
    ...
    const bob_Deposit_After = await stabilityPool.getCompoundedBUSDCDeposit(bob)
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { busdcAmount: A_busdcAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraBUSDCAmount: toBN(dec(300, 18)), extraParams: { from: alice } })
    const { busdcAmount: B_busdcAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraBUSDCAmount: toBN(dec(200, 18)), extraParams: { from: bob } })
    const { busdcAmount: C_busdcAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraBUSDCAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Liquidate A, B and C
    const activeBUSDCDebt_0 = await activePool.getBUSDCDebt()
    const defaultBUSDCDebt_0 = await defaultPool.getBUSDCDebt()

    await troveManager.liquidate(alice)
    const activeBUSDCDebt_A = await activePool.getBUSDCDebt()
    const defaultBUSDCDebt_A = await defaultPool.getBUSDCDebt()

    await troveManager.liquidate(bob)
    const activeBUSDCDebt_B = await activePool.getBUSDCDebt()
    const defaultBUSDCDebt_B = await defaultPool.getBUSDCDebt()
    ...
    assert.equal((await busdcToken.balanceOf(alice)).toString(), A_busdcAmount)
    assert.equal((await busdcToken.balanceOf(bob)).toString(), B_busdcAmount)
    assert.equal((await busdcToken.balanceOf(carol)).toString(), C_busdcAmount)
  })

  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    await openTrove({ ICR: toBN(dec(8, 18)), extraBUSDCAmount: toBN(dec(100, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(221, 16)), extraBUSDCAmount: toBN(dec(100, 18)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraBUSDCAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Defaulter opens with 60 BUSDC, 0.6 ETH
    ...
    /* Liquidate defaulter. 30 BUSDC and 0.3 ETH is distributed between A, B and C.

    A receives (30 * 2/4) = 15 BUSDC, and (0.3*2/4) = 0.15 ETH
    B receives (30 * 1/4) = 7.5 BUSDC, and (0.3*1/4) = 0.075 ETH
    C receives (30 * 1/4) = 7.5 BUSDC, and (0.3*1/4) = 0.075 ETH
    */
    await troveManager.liquidate(defaulter_1)
  ...
  it("liquidate(): when SP > 0, triggers B2B reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalBUSDCDeposits(), dec(100, 18))

    const G_Before = await stabilityPool.epochToScaleToG(0, 0)

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

    // Price drops to 1ETH:100BUSDC, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
    assert.isFalse(await th.checkRecoveryMode(contracts))
    ...
    // Expect G has increased from the B2B reward event triggered
  ...
  it("liquidate(): when SP is empty, doesn't update G", async () => {
    ...
    // Check SP is empty
    assert.equal((await stabilityPool.getTotalBUSDCDeposits()), '0')
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    // A adds 10 BUSDC to the SP, but less than C's debt
    ...
    await borrowerOperations.repayBUSDC(dec(1, 18), D, D, {from: D})
    await borrowerOperations.repayBUSDC(dec(1, 18), E, E, {from: E})
    ...
    const pendingBUSDCDebt_C = await troveManager.getPendingBUSDCDebtReward(C)
    const defaultPoolETH = await defaultPool.getETH()
    const defaultPoolBUSDCDebt = await defaultPool.getBUSDCDebt()
    assert.isTrue(pendingETH_C.lte(defaultPoolETH))
    assert.isTrue(pendingBUSDCDebt_C.lte(defaultPoolBUSDCDebt))
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000)
    assert.isAtMost(th.getDifference(pendingBUSDCDebt_C, defaultPoolBUSDCDebt), 1000)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1ETH:100BUSDC, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 BUSDC and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 BUSDC, 0.1 ETH
    await troveManager.liquidate(defaulter_1)
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraBUSDCAmount: dec(1, 24), extraParams: { from: whale } })
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await busdcToken.balanceOf(dennis)
    const E_balanceBefore = await busdcToken.balanceOf(erin)
    const F_balanceBefore = await busdcToken.balanceOf(flyn)
    ...
    assert.equal((await busdcToken.balanceOf(dennis)).toString(), D_balanceBefore)
    assert.equal((await busdcToken.balanceOf(erin)).toString(), E_balanceBefore)
    assert.equal((await busdcToken.balanceOf(flyn)).toString(), F_balanceBefore)
  })

  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 BUSDC to SP
    await openTrove({ ICR: toBN(dec(100, 18)), extraBUSDCAmount: toBN(dec(500, 18)), extraParams: { from: whale } })
    ...
    // Check pool has 500 BUSDC
    assert.equal((await stabilityPool.getTotalBUSDCDeposits()).toString(), dec(500, 18))
    ...
    assert.equal((await stabilityPool.getTotalBUSDCDeposits()).toString(), '0')
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    // Whale provides 400 BUSDC to the SP
    const whaleDeposit = toBN(dec(40000, 18))
    await openTrove({ ICR: toBN(dec(100, 18)), extraBUSDCAmount: whaleDeposit, extraParams: { from: whale } })
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraBUSDCAmount: A_deposit, extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraBUSDCAmount: B_deposit, extraParams: { from: bob } })
    ...
    // Check 800 BUSDC in Pool
    const totalDeposits = whaleDeposit.add(A_deposit).add(B_deposit)
    assert.equal((await stabilityPool.getTotalBUSDCDeposits()).toString(), totalDeposits)
    ...
    Total BUSDC in Pool: 800 BUSDC

    Then, liquidation hits A,B,C:

    Total liquidated debt = 150 + 350 + 150 = 650 BUSDC
    Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

    whale busdc loss: 650 * (400/800) = 325 busdc
    alice busdc loss:  650 *(100/800) = 81.25 busdc
    bob busdc loss: 650 * (300/800) = 243.75 busdc

    whale remaining deposit: (400 - 325) = 75 busdc
    alice remaining deposit: (100 - 81.25) = 18.75 busdc
    bob remaining deposit: (300 - 243.75) = 56.25 busdc
    ...
    Total remaining deposits: 150 BUSDC
    Total ETH gain: 4.975 ETH */

    // Check remaining BUSDC Deposits and ETH gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedBUSDCDeposit(whale)
    const alice_Deposit_After = await stabilityPool.getCompoundedBUSDCDeposit(alice)
    const bob_Deposit_After = await stabilityPool.getCompoundedBUSDCDeposit(bob)
    ...
    const total_BUSDCinSP = (await stabilityPool.getTotalBUSDCDeposits()).toString()
    const total_ETHinSP = (await stabilityPool.getETH()).toString()

    assert.isAtMost(th.getDifference(total_BUSDCinSP, totalDeposits.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): when SP > 0, triggers B2B reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraBUSDCAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
  ...
  it('batchLiquidateTroves(): closes every trove with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1ETH:100BUSDC, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: toBN(dec(500, 18)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: toBN(dec(500, 18)), extraParams: { from: erin } })
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraBUSDCAmount: spDeposit, extraParams: { from: whale } })
    ...
    th.assertIsApproximatelyEqual((await stabilityPool.getTotalBUSDCDeposits()).toString(), spDeposit.sub(A_debt).sub(B_debt))
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    await busdcToken.transfer(carol, dec(100, 18), { from: whale })
  ...
  it("batchLiquidateTroves: when SP > 0, triggers B2B reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraBUSDCAmount: partialRedemptionAmount, extraParams: { from: alice } })
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra BUSDC. At least 3 iterations are needed
  ...
  it('redeemCollateral(): cancels the provided BUSDC with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraBUSDCAmount: dec(10, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(290, 16)), extraBUSDCAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraBUSDCAmount: dec(10, 18), extraParams: { from: carol } })
    const partialRedemptionAmount = toBN(2)
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount)
    // start Dennis with a high ICR
    await openTrove({ ICR: toBN(dec(100, 18)), extraBUSDCAmount: redemptionAmount, extraParams: { from: dennis } })

    const dennis_ETHBalance_Before = toBN(await contracts.backedToken.balanceOf(dennis))

    const dennis_BUSDCBalance_Before = await busdcToken.balanceOf(dennis)
    ...
    // Find hints for redeeming 20 BUSDC
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(redemptionAmount, price, 0)
    ...
    /* check that Dennis' redeemed 20 BUSDC has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) BUSDC debt + 50 for gas compensation. */
    th.assertIsApproximatelyEqual(alice_debt_After, A_totalDebt.sub(partialRedemptionAmount))
    assert.equal(bob_debt_After, '0')
    assert.equal(carol_debt_After, '0')

    const dennis_ETHBalance_After = toBN(await contracts.backedToken.balanceOf(dennis))
    const receivedETH = dennis_ETHBalance_After.sub(dennis_ETHBalance_Before)

    const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)) // convert redemptionAmount BUSDC to ETH, at ETH:USD price 200
    ...
    const dennis_BUSDCBalance_After = (await busdcToken.balanceOf(dennis)).toString()
    assert.equal(dennis_BUSDCBalance_After, dennis_BUSDCBalance_Before.sub(redemptionAmount))
  })
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraBUSDCAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraBUSDCAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraBUSDCAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraBUSDCAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraBUSDCAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST ---

    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 BUSDC), 20000%
    const { busdcAmount: F_busdcAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraBUSDCAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 BUSDC
    const flynBalance = await busdcToken.balanceOf(flyn)
    th.assertIsApproximatelyEqual(flynBalance, F_busdcAmount.sub(redemptionAmount))
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraBUSDCAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraBUSDCAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraBUSDCAmount: dec(20, 18), extraParams: { from: carol } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 BUSDC
    const flynBalance = (await busdcToken.balanceOf(flyn)).toString()
    th.assertIsApproximatelyEqual(flynBalance, F_busdcAmount.sub(redemptionAmount))
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    await contracts.backedToken.approve(contracts.borrowerOperations.address, dec(1000, 'ether'), {from: A})
    await contracts.backedToken.approve(contracts.borrowerOperations.address, dec(1000, 'ether'), {from: B})
    await contracts.backedToken.approve(contracts.borrowerOperations.address, dec(1000, 'ether'), {from: C})
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveBUSDCAmount(dec(10000, 18)), A, A, dec(1000, 'ether'), { from: A })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveBUSDCAmount(dec(20000, 18)), B, B, dec(1000, 'ether'), { from: B })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveBUSDCAmount(dec(30000, 18)), C, C, dec(1000, 'ether'), { from: C })

    // A and C send all their tokens to B
    await busdcToken.transfer(B, await busdcToken.balanceOf(A), {from: A})
    await busdcToken.transfer(B, await busdcToken.balanceOf(C), {from: C})
    ...
    // BUSDC redemption is 55000 US
    const BUSDCRedemption = dec(55000, 18)
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, BUSDCRedemption, th._100pct)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    ...
    await contracts.backedToken.approve(contracts.borrowerOperations.address, dec(1000, 'ether'), {from: B})
    await contracts.backedToken.approve(contracts.borrowerOperations.address, dec(1000, 'ether'), {from: C})
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveBUSDCAmount(dec(6000, 18)), A, A, dec(1000, 'ether'), { from: A })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveBUSDCAmount(dec(20000, 18)), B, B, dec(1000, 'ether'), { from: B })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveBUSDCAmount(dec(30000, 18)), C, C, dec(1000, 'ether'), { from: C })
    ...
    // BUSDC redemption is 55000 BUSDC
    const BUSDCRedemption = dec(55000, 18)
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, BUSDCRedemption, th._100pct)
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(363, 16)), extraBUSDCAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraBUSDCAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(333, 16)), extraBUSDCAmount: dec(10, 18), extraParams: { from: carol } })
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 BUSDC from Carol's Trove
    ...
    // Dennis tries to redeem 20 BUSDC
    const redemptionTx = await troveManager.redeemCollateral(
      redemptionAmount,
      firstRedemptionHint,
      upperPartialRedemptionHint,
    ...
    // Since Alice already redeemed 1 BUSDC from Carol's Trove, Dennis was  able to redeem:
    //  - 9 BUSDC from Carol's
    //  - 8 BUSDC from Bob's
    // for a total of 17 BUSDC.

    // Dennis calculated his hint for redeeming 2 BUSDC from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 BUSDC to fully complete his redemption of 20 BUSDC.
    ...
    const expectedTotalETHDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)) // redempted BUSDC converted to ETH, at ETH:USD price 200
    ...
    const dennis_BUSDCBalance_After = (await busdcToken.balanceOf(dennis)).toString()
    th.assertIsApproximatelyEqual(dennis_BUSDCBalance_After, dennis_BUSDCBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion)))
  })
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveBUSDCAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraBUSDCAmount: amount, extraParams: { from: bob } })

    await busdcToken.transfer(carol, amount, { from: bob })
    ...
    const carol_ETHBalance_Before = toBN(await contracts.backedToken.balanceOf(carol))
    ...
    const carol_ETHBalance_After = toBN(await contracts.backedToken.balanceOf(carol))

    const expectedTotalETHDrawn = toBN(amount).div(toBN(100)) // convert 100 BUSDC to ETH at ETH:USD price of 100
    ...
    const carol_BUSDCBalance_After = (await busdcToken.balanceOf(carol)).toString()
    assert.equal(carol_BUSDCBalance_After, '0')
  })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { busdcAmount: B_busdcAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraBUSDCAmount: A_debt, extraParams: { from: bob } })

    await busdcToken.transfer(carol, B_busdcAmount, { from: bob })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: amount1, extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: amount1, extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: amount1, extraParams: { from: carol } })
    const redemptionAmount = C_totalDebt.add(B_totalDebt).add(A_totalDebt)
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraBUSDCAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraBUSDCAmount: dec(10, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500BUSDC to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: dec(500, 18), extraParams: { from: alice } })
    await busdcToken.transfer(erin, dec(500, 18), { from: alice })
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraBUSDCAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraBUSDCAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraBUSDCAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraBUSDCAmount: dec(40, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraBUSDCAmount: dec(80, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraBUSDCAmount: dec(90, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)

    // Check total BUSDC supply
    ...
    // BUSDC redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedBUSDCRedemption = expectedTotalSupply.div(toBN(10))

    // Max fee is <5%
    const lessThan5pct = '49999999999999999'
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedBUSDCRedemption, lessThan5pct), "Fee exceeded provided maximum")

    await troveManager.setBaseRate(0)  // artificially zero the baseRate

    // Max fee is 1%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedBUSDCRedemption, dec(1, 16)), "Fee exceeded provided maximum")

    await troveManager.setBaseRate(0)

     // Max fee is 3.754%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedBUSDCRedemption, dec(3754, 13)), "Fee exceeded provided maximum")

    await troveManager.setBaseRate(0)

    // Max fee is 0.5%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedBUSDCRedemption, dec(5, 15)), "Fee exceeded provided maximum")
  })

  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraBUSDCAmount: dec(9500, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(395, 16)), extraBUSDCAmount: dec(9000, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(390, 16)), extraBUSDCAmount: dec(10000, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    ...
    // BUSDC redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedBUSDCRedemption = expectedTotalSupply.div(toBN(10))

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice()
    const ETHDrawn = attemptedBUSDCRedemption.mul(mv._1e18BN).div(price)
    const slightlyMoreThanFee = (await troveManager.getRedemptionFeeWithDecay(ETHDrawn))
    const tx1 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedBUSDCRedemption, slightlyMoreThanFee)
    ...
    const tx2 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedBUSDCRedemption, exactSameFee)
    assert.isTrue(tx2.receipt.status)

    await troveManager.setBaseRate(0)

     // Max fee is 10%
    const tx3 = await th.redeemCollateralAndGetTxObject(B, contracts, attemptedBUSDCRedemption, dec(1, 17))
    assert.isTrue(tx3.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 37.659%
    const tx4 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedBUSDCRedemption, dec(37659, 13))
    assert.isTrue(tx4.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 100%
    const tx5 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedBUSDCRedemption, dec(1, 18))
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
    ...
    // B, C, D, F open trove
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraBUSDCAmount: dec(200, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraBUSDCAmount: dec(400, 18), extraParams: { from: dennis } })
    const { totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: flyn } })

    const redemptionAmount = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(F_totalDebt)
    // Alice opens trove and transfers BUSDC to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(300, 16)), extraBUSDCAmount: redemptionAmount, extraParams: { from: alice } })
    await busdcToken.transfer(erin, redemptionAmount, { from: alice })
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedBUSDCDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedBUSDCDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedBUSDCDeposit(dennis)).toString()

    const bob_ETHGain_before = (await stabilityPool.getDepositorETHGain(bob)).toString()
    const carol_ETHGain_before = (await stabilityPool.getDepositorETHGain(carol)).toString()
    const dennis_ETHGain_before = (await stabilityPool.getDepositorETHGain(dennis)).toString()

    // Check the remaining BUSDC and ETH in Stability Pool after liquidation is non-zero
    const BUSDCinSP = await stabilityPool.getTotalBUSDCDeposits()
    const ETHinSP = await stabilityPool.getETH()
    assert.isTrue(BUSDCinSP.gte(mv._zeroBN))
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedBUSDCDeposit(bob)).toString()
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedBUSDCDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedBUSDCDeposit(dennis)).toString()
  ...
  it("redeemCollateral(): caller can redeem their entire BUSDCToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 400 BUSDC to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraBUSDCAmount: dec(400, 18), extraParams: { from: alice } })
    await busdcToken.transfer(erin, dec(400, 18), { from: alice })

    // Check Erin's balance before
    const erin_balance_before = await busdcToken.balanceOf(erin)
    assert.equal(erin_balance_before, dec(400, 18))

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraBUSDCAmount: dec(590, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraBUSDCAmount: dec(1990, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(500, 16)), extraBUSDCAmount: dec(1990, 18), extraParams: { from: dennis } })
    ...
    // Erin attempts to redeem 400 BUSDC
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    ...
    // Check activePool debt reduced by  400 BUSDC
    const activePool_debt_after = await activePool.getBUSDCDebt()
    ...
    const erin_balance_after = (await busdcToken.balanceOf(erin)).toString()
    assert.equal(erin_balance_after, '0')
  })

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's BUSDC token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    ...
    // Erin tries to redeem 1000 BUSDC
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
      ...
      assert.include(error.message, "Requested redemption amount must be <= user's BUSDC token balance")
    }

    // Erin tries to redeem 401 BUSDC
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 239482309 BUSDC
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 2^256 - 1 BUSDC
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed BUSDC (assuming 1 BUSDC has value of $1)", async () => {
    const { collateral: W_coll } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 1000 BUSDC each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraBUSDCAmount: dec(4990, 18), extraParams: { from: alice } })
    await busdcToken.transfer(erin, dec(1000, 18), { from: alice })
    await busdcToken.transfer(flyn, dec(1000, 18), { from: alice })
    await busdcToken.transfer(graham, dec(1000, 18), { from: alice })

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraBUSDCAmount: dec(1590, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(600, 16)), extraBUSDCAmount: dec(1090, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(800, 16)), extraBUSDCAmount: dec(1090, 18), extraParams: { from: dennis } })

    const totalColl = W_coll.add(A_coll).add(B_coll).add(C_coll).add(D_coll)

    const price = await priceFeed.getPrice()

    const _120_BUSDC = '120000000000000000000'
    const _373_BUSDC = '373000000000000000000'
    const _950_BUSDC = '950000000000000000000'
    ...
    // Erin redeems 120 BUSDC
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_120_BUSDC, price, 0))
    ...
    /* 120 BUSDC redeemed.  Expect $120 worth of ETH removed. At ETH:USD price of $200,
    ETH removed = (120/200) = 0.6 ETH
    Total active ETH = 280 - 0.6 = 279.4 ETH */

    const activeETH_1 = await activePool.getETH()
    assert.equal(activeETH_1.toString(), activeETH_0.sub(toBN(_120_BUSDC).mul(mv._1e18BN).div(price)));

    // Flyn redeems 373 BUSDC
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_373_BUSDC, price, 0))
    ...
    /* 373 BUSDC redeemed.  Expect $373 worth of ETH removed. At ETH:USD price of $200,
    ETH removed = (373/200) = 1.865 ETH
    Total active ETH = 279.4 - 1.865 = 277.535 ETH */
    const activeETH_2 = await activePool.getETH()
    assert.equal(activeETH_2.toString(), activeETH_1.sub(toBN(_373_BUSDC).mul(mv._1e18BN).div(price)));

    // Graham redeems 950 BUSDC
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_950_BUSDC, price, 0))
    ...
    /* 950 BUSDC redeemed.  Expect $950 worth of ETH removed. At ETH:USD price of $200,
    ETH removed = (950/200) = 4.75 ETH
    Total active ETH = 277.535 - 4.75 = 272.785 ETH */
    const activeETH_3 = (await activePool.getETH()).toString()
    assert.equal(activeETH_3.toString(), activeETH_2.sub(toBN(_950_BUSDC).mul(mv._1e18BN).div(price)));
  })
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint BUSDC to Bob
    await busdcToken.unprotectedMint(bob, dec(100, 18))

    assert.equal((await busdcToken.balanceOf(bob)), dec(100, 18))
    ...
    // Bob tries to redeem his illegally obtained BUSDC
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint BUSDC to Bob
    await busdcToken.unprotectedMint(bob, '101000000000000000000')

    assert.equal((await busdcToken.balanceOf(bob)), '101000000000000000000')

    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraBUSDCAmount: dec(40, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraBUSDCAmount: dec(40, 18), extraParams: { from: dennis } })

    const totalDebt = C_totalDebt.add(D_totalDebt)
    th.assertIsApproximatelyEqual((await activePool.getBUSDCDebt()).toString(), totalDebt)
    ...
    // Bob attempts to redeem his ill-gotten 101 BUSDC, from a system that has 100 BUSDC outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: C } })
    ...
    const A_balanceBefore = await busdcToken.balanceOf(A)

    await th.redeemCollateral(A, contracts, dec(10, 18), GAS_PRICE)

    // Check A's balance has decreased by 10 BUSDC
    assert.equal(await busdcToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    const A_balanceBefore = await busdcToken.balanceOf(A)
    const B_balanceBefore = await busdcToken.balanceOf(B)
    ...
    // Check B's balance has decreased by 10 BUSDC
    assert.equal(await busdcToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 BUSDC
    assert.equal(A_balanceBefore.sub(await busdcToken.balanceOf(A)), dec(10, 18))
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to B2B staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 B2B
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await b2bToken.approve(b2bStaking.address, dec(1, 18), { from: multisig })
    await b2bStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check B2B Staking contract balance before is zero
    const b2bStakingBalance_Before = await contracts.backedToken.balanceOf(b2bStaking.address)
    assert.equal(b2bStakingBalance_Before, '0')
    ...
    // Check B2B Staking contract balance after is non-zero
    const b2bStakingBalance_After = toBN(await contracts.backedToken.balanceOf(b2bStaking.address))
    assert.isTrue(b2bStakingBalance_After.gt(toBN('0')))
  })

  it("redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-B2B-staked in B2B Staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 B2B
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await b2bToken.approve(b2bStaking.address, dec(1, 18), { from: multisig })
    await b2bStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check B2B Staking ETH-fees-per-B2B-staked before is zero
    const F_ETH_Before = await b2bStaking.F_ETH()
    ...
    // Check B2B Staking ETH-fees-per-B2B-staked after is non-zero
    const F_ETH_After = await b2bStaking.F_ETH()
    assert.isTrue(F_ETH_After.gt('0'))
  })

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to B2B staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 B2B
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await b2bToken.approve(b2bStaking.address, dec(1, 18), { from: multisig })
    await b2bStaking.stake(dec(1, 18), { from: multisig })
    ...
    const b2bStakingBalance_Before = toBN(await contracts.backedToken.balanceOf(b2bStaking.address))
    ...
    // check B2B Staking balance has increased
    assert.isTrue(b2bStakingBalance_After.gt(b2bStakingBalance_Before))
  })

  it("redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-B2B-staked in the staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 B2B
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await b2bToken.approve(b2bStaking.address, dec(1, 18), { from: multisig })
    await b2bStaking.stake(dec(1, 18), { from: multisig })
  ...
  it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: C } })
    const totalDebt = W_totalDebt.add(A_totalDebt).add(B_totalDebt).add(C_totalDebt)

    const A_balanceBefore = toBN(await contracts.backedToken.balanceOf(A))
    ...
    // Check total BUSDC supply
    const activeBUSDC = await activePool.getBUSDCDebt()
    const defaultBUSDC = await defaultPool.getBUSDCDebt()

    const totalBUSDCSupply = activeBUSDC.add(defaultBUSDC)
    th.assertIsApproximatelyEqual(totalBUSDCSupply, totalDebt)
    ...
    */
    ...
    const A_balanceAfter = toBN(await contracts.backedToken.balanceOf(A))
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    const { netDebt: W_netDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraBUSDCAmount: dec(10000, 18), extraParams: { from: whale } })

    const { netDebt: A_netDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))

    const A_balanceBefore = toBN(await contracts.backedToken.balanceOf(A))
    const B_balanceBefore = toBN(await contracts.backedToken.balanceOf(B))
    const C_balanceBefore = toBN(await contracts.backedToken.balanceOf(C))

    // whale redeems 360 BUSDC.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt, collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))
    ...
    const B_balanceBefore = toBN(await contracts.backedToken.balanceOf(B))
    const C_balanceBefore = toBN(await contracts.backedToken.balanceOf(C))
    const D_balanceBefore = toBN(await contracts.backedToken.balanceOf(D))
    ...
    // whale redeems BUSDC.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount, GAS_PRICE)
    ...
    */
    ...
    const A_balanceAfter = toBN(await contracts.backedToken.balanceOf(A))
    const B_balanceAfter = toBN(await contracts.backedToken.balanceOf(B))
    const C_balanceAfter = toBN(await contracts.backedToken.balanceOf(C))
    const D_balanceAfter = toBN(await contracts.backedToken.balanceOf(D))
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: C } })
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(280, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems BUSDC.  Expect this to fully redeem A, B, C, and partially redeem 15 BUSDC from D.
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's ETH surplus (collateral - ETH drawn) available for the trove owner after re-opening trove", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraBUSDCAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    await contracts.backedToken.approve(contracts.borrowerOperations.address, dec(1000, 'ether'), {from: alice})
    const { busdcAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraBUSDCAmount: dec(1, 24), extraParams: { from: alice } })
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const {
        firstRedemptionHint,
        partialRedemptionHintNICR
      } = await hintHelpers.getRedemptionHints(busdcAmount, price, 0)
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
      await contracts.backedToken.approve(contracts.borrowerOperations.address, busdcAmount.mul(mv._1e18BN).div(price), {from: alice})
      await borrowerOperations.adjustTrove(th._100pct, 0, busdcAmount, true, alice, alice, busdcAmount.mul(mv._1e18BN).div(price), { from: alice })
    }
  ...
  it("getPendingBUSDCDebtReward(): Returns 0 if there is no pending BUSDCDebt reward", async () => {
    // Make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraBUSDCAmount: dec(100, 18), extraParams: { from: defaulter_1 } })

    await openTrove({ ICR: toBN(dec(3, 18)), extraBUSDCAmount: dec(20, 18), extraParams: { from: carol } })

    await openTrove({ ICR: toBN(dec(20, 18)), extraBUSDCAmount: totalDebt, extraParams: { from: whale } })
    ...
    const current_L_BUSDCDebt = await troveManager.L_BUSDCDebt()
    assert.equal(current_L_BUSDCDebt, 0)

    const carolSnapshot_L_BUSDCDebt = (await troveManager.rewardSnapshots(carol))[1]
    assert.equal(carolSnapshot_L_BUSDCDebt, 0)

    const carol_PendingBUSDCDebtReward = await troveManager.getPendingBUSDCDebtReward(carol)
    assert.equal(carol_PendingBUSDCDebtReward, 0)
  })
  ...
  it("computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 BUSDC", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
    const debt = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 BUSDC", async () => {
    const price = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 BUSDC", async () => {
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 BUSDC", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraBUSDCAmount: B_totalDebt, extraParams: { from: A } })

    // to be able to repay:
    await busdcToken.transfer(B, B_totalDebt, { from: A })
