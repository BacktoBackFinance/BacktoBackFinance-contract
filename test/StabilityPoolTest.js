...
contract('StabilityPool', async accounts => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    beforeEach(async () => {
      ...
      await contracts.backedToken.mint(whale, dec(100000, 'ether'))
      await contracts.backedToken.mint(alice, dec(100000, 'ether'))
      await contracts.backedToken.mint(bob, dec(100000, 'ether'))
      await contracts.backedToken.mint(carol, dec(100000, 'ether'))
      await contracts.backedToken.mint(dennis, dec(100000, 'ether'))
      await contracts.backedToken.mint(erin, dec(100000, 'ether'))
      await contracts.backedToken.mint(flyn, dec(100000, 'ether'))

      await contracts.backedToken.mint(A, dec(100000, 'ether'))
      await contracts.backedToken.mint(B, dec(100000, 'ether'))
      await contracts.backedToken.mint(C, dec(100000, 'ether'))
      await contracts.backedToken.mint(D, dec(100000, 'ether'))
      await contracts.backedToken.mint(E, dec(100000, 'ether'))
      await contracts.backedToken.mint(F, dec(100000, 'ether'))

      await contracts.backedToken.mint(defaulter_1, dec(100000, 'ether'))
      await contracts.backedToken.mint(defaulter_2, dec(100000, 'ether'))
      await contracts.backedToken.mint(defaulter_3, dec(100000, 'ether'))
    ...
    // --- provideToSP() ---
    // increases recorded BUSDC at Stability Pool
    it("provideToSP(): increases the Stability Pool BUSDC balance", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({ extraBUSDCAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      // check BUSDC balances after
      const stabilityPool_BUSDC_After = await stabilityPool.getTotalBUSDCDeposits()
      assert.equal(stabilityPool_BUSDC_After, 200)
    })
    ...
    it("provideToSP(): reduces the user's BUSDC balance by the correct amount", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({ extraBUSDCAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      // check user's BUSDC balance change
      const alice_BUSDCBalance_After = await busdcToken.balanceOf(alice)

      assert.equal(alice_BUSDCBalance_Before.sub(alice_BUSDCBalance_After), '200')
    })

    it("provideToSP(): increases totalBUSDCDeposits by correct amount", async () => {
      // --- SETUP ---

      // Whale opens Trove with 50 ETH, adds 2000 BUSDC to StabilityPool
      await openTrove({ extraBUSDCAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(2000, 18), frontEnd_1, { from: whale })

      const totalBUSDCDeposits = await stabilityPool.getTotalBUSDCDeposits()
      assert.equal(totalBUSDCDeposits, dec(2000, 18))
    })
    ...
    it('provideToSP(): Correctly updates user snapshots of accumulated rewards per unit staked', async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      const whaleBUSDC = await busdcToken.balanceOf(whale)
      await stabilityPool.provideToSP(whaleBUSDC, frontEnd_1, { from: whale })

      // 2 Troves opened, each withdraws minimum debt
      await openTrove({ extraBUSDCAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1, } })
      await openTrove({ extraBUSDCAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2, } })

      // Alice makes Trove and withdraws 100 BUSDC
      await openTrove({ extraBUSDCAmount: toBN(dec(100, 18)), ICR: toBN(dec(5, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })


      // price drops: defaulter's Troves fall below MCR, whale doesn't
      await priceFeed.setPrice(dec(105, 18));

      const SPBUSDC_Before = await stabilityPool.getTotalBUSDCDeposits()
      ...
      const SPBUSDC_After = await stabilityPool.getTotalBUSDCDeposits()
      assert.isTrue(SPBUSDC_After.lt(SPBUSDC_Before))
    ...
    it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
      ...
      // 3 Troves opened. Two users withdraw 160 BUSDC each
      await openTrove({ extraBUSDCAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1, value: dec(50, 'ether') } })
      await openTrove({ extraBUSDCAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2, value: dec(50, 'ether') } })
      await openTrove({ extraBUSDCAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_3, value: dec(50, 'ether') } })

      // --- TEST ---

      // Alice makes deposit #1: 150 BUSDC
      await openTrove({ extraBUSDCAmount: toBN(dec(250, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
      ...
      // 2 users with Trove with 180 BUSDC drawn are closed
      await troveManager.liquidate(defaulter_1, { from: owner })  // 180 BUSDC closed
      await troveManager.liquidate(defaulter_2, { from: owner }) // 180 BUSDC closed

      const alice_compoundedDeposit_1 = await stabilityPool.getCompoundedBUSDCDeposit(alice)
      ...
      // Bob withdraws BUSDC and deposits to StabilityPool
      await openTrove({ extraBUSDCAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await stabilityPool.provideToSP(dec(427, 18), frontEnd_1, { from: alice })

      // Defaulter 3 Trove is closed
      await troveManager.liquidate(defaulter_3, { from: owner })

      const alice_compoundedDeposit_2 = await stabilityPool.getCompoundedBUSDCDeposit(alice)
      ...
      // Alice makes deposit #3:  100BUSDC
      await stabilityPool.provideToSP(dec(100, 18), frontEnd_1, { from: alice })
    ...
    it("provideToSP(): reverts if user tries to provide more than their BUSDC balance", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })

      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob, value: dec(50, 'ether') } })
      const aliceBUSDCbal = await busdcToken.balanceOf(alice)
      const bobBUSDCbal = await busdcToken.balanceOf(bob)

      // Alice, attempts to deposit 1 wei more than her balance

      const aliceTxPromise = stabilityPool.provideToSP(aliceBUSDCbal.add(toBN(1)), frontEnd_1, { from: alice })
      await assertRevert(aliceTxPromise, "revert")

      // Bob, attempts to deposit 235534 more than his balance

      const bobTxPromise = stabilityPool.provideToSP(bobBUSDCbal.add(toBN(dec(235534, 18))), frontEnd_1, { from: bob })
      await assertRevert(bobTxPromise, "revert")
    })

    it("provideToSP(): reverts if user tries to provide 2^256-1 BUSDC, which exceeds their balance", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob, value: dec(50, 'ether') } })
      ...
      // Alice attempts to deposit 2^256-1 BUSDC
      try {
    ...
    it("provideToSP(): reverts if cannot receive ETH Gain", async () => {
      // --- SETUP ---
      // Whale deposits 1850 BUSDC in StabilityPool
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      ...
      await busdcToken.transfer(nonPayable.address, dec(250, 18), { from: whale })

      // NonPayable makes deposit #1: 150 BUSDC
      ...
      // NonPayable tries to make deposit #2: 100BUSDC (which also attempts to withdraw ETH gain)
    ...
    it("provideToSP(): doesn't impact other users' deposits or ETH gains", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraBUSDCAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraBUSDCAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // D opens a trove
      await openTrove({ extraBUSDCAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      ...
      const alice_BUSDCDeposit_Before = (await stabilityPool.getCompoundedBUSDCDeposit(alice)).toString()
      const bob_BUSDCDeposit_Before = (await stabilityPool.getCompoundedBUSDCDeposit(bob)).toString()
      const carol_BUSDCDeposit_Before = (await stabilityPool.getCompoundedBUSDCDeposit(carol)).toString()
      ...
      //check non-zero BUSDC and ETHGain in the Stability Pool
      const BUSDCinSP = await stabilityPool.getTotalBUSDCDeposits()
      const ETHinSP = await stabilityPool.getETH()
      assert.isTrue(BUSDCinSP.gt(mv._zeroBN))
      assert.isTrue(ETHinSP.gt(mv._zeroBN))

      // D makes an SP deposit
      await stabilityPool.provideToSP(dec(1000, 18), frontEnd_1, { from: dennis })
      assert.equal((await stabilityPool.getCompoundedBUSDCDeposit(dennis)).toString(), dec(1000, 18))

      const alice_BUSDCDeposit_After = (await stabilityPool.getCompoundedBUSDCDeposit(alice)).toString()
      const bob_BUSDCDeposit_After = (await stabilityPool.getCompoundedBUSDCDeposit(bob)).toString()
      const carol_BUSDCDeposit_After = (await stabilityPool.getCompoundedBUSDCDeposit(carol)).toString()
      ...
      assert.equal(alice_BUSDCDeposit_Before, alice_BUSDCDeposit_After)
      assert.equal(bob_BUSDCDeposit_Before, bob_BUSDCDeposit_After)
      assert.equal(carol_BUSDCDeposit_Before, carol_BUSDCDeposit_After)
    ...
    it("provideToSP(): doesn't impact system debt, collateral or TCR", async () => {
      ...
      // D opens a trove
      await openTrove({ extraBUSDCAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      ...
      const activeDebt_Before = (await activePool.getBUSDCDebt()).toString()
      const defaultedDebt_Before = (await defaultPool.getBUSDCDebt()).toString()
      ...
      const activeDebt_After = (await activePool.getBUSDCDebt()).toString()
      const defaultedDebt_After = (await defaultPool.getBUSDCDebt()).toString()
    ...
    it("provideToSP(): doesn't impact any troves, including the caller's trove", async () => {
      ...
      // D opens a trove
      await openTrove({ extraBUSDCAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
    ...
    it("provideToSP(): doesn't protect the depositor's trove from liquidation", async () => {
      ...
      assert.equal((await stabilityPool.getCompoundedBUSDCDeposit(bob)).toString(), dec(1000, 18))
    ...
    it("provideToSP(): providing 0 BUSDC reverts", async () => {
      // --- SETUP ---
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      ...
      // A, B, C provides 100, 50, 30 BUSDC to SP
      await stabilityPool.provideToSP(dec(100, 18), frontEnd_1, { from: alice })
      ...
      const BUSDCinSP_Before = (await stabilityPool.getTotalBUSDCDeposits()).toString()

      assert.equal(BUSDCinSP_Before, dec(180, 18))

      // Bob provides 0 BUSDC to the Stability Pool
      const txPromise_B = stabilityPool.provideToSP(0, frontEnd_1, { from: bob })
      await th.assertRevert(txPromise_B)
    })

    // --- B2B functionality ---
    it("provideToSP(), new deposit: when SP > 0, triggers B2B reward event - increases the sum G", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })

      // A, B, C open troves and make Stability Pool deposits
      await openTrove({ extraBUSDCAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraBUSDCAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraBUSDCAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("provideToSP(), new deposit: sets the correct front end tag", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraBUSDCAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), new deposit: depositor does not receive any B2B rewards", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      ...
      // Get A, B, C B2B balances before and confirm they're zero
      const A_B2BBalance_Before = await b2bToken.balanceOf(A)
      const B_B2BBalance_Before = await b2bToken.balanceOf(B)

      assert.equal(A_B2BBalance_Before, '0')
      assert.equal(B_B2BBalance_Before, '0')
      ...
      // Get A, B, C B2B balances after, and confirm they're still zero
      const A_B2BBalance_After = await b2bToken.balanceOf(A)
      const B_B2BBalance_After = await b2bToken.balanceOf(B)

      assert.equal(A_B2BBalance_After, '0')
      assert.equal(B_B2BBalance_After, '0')
    })

    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive any B2B rewards", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraBUSDCAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(4000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })

      // --- SETUP ---

      const initialDeposit_A = await busdcToken.balanceOf(A)
      const initialDeposit_B = await busdcToken.balanceOf(B)
      // A, B provide to SP
      ...
      // Get A, B, C B2B balances before and confirm they're non-zero
      const A_B2BBalance_Before = await b2bToken.balanceOf(A)
      const B_B2BBalance_Before = await b2bToken.balanceOf(B)
      assert.isTrue(A_B2BBalance_Before.gt(toBN('0')))
      assert.isTrue(B_B2BBalance_Before.gt(toBN('0')))
      ...
      // Get A, B, C B2B balances after, and confirm they have not changed
      const A_B2BBalance_After = await b2bToken.balanceOf(A)
      const B_B2BBalance_After = await b2bToken.balanceOf(B)

      assert.isTrue(A_B2BBalance_After.eq(A_B2BBalance_Before))
      assert.isTrue(B_B2BBalance_After.eq(B_B2BBalance_Before))
    })

    it("provideToSP(), new eligible deposit: tagged front end receives B2B rewards", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraBUSDCAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraBUSDCAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraBUSDCAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
      ...
      // Get F1, F2, F3 B2B balances before, and confirm they're zero
      const frontEnd_1_B2BBalance_Before = await b2bToken.balanceOf(frontEnd_1)
      const frontEnd_2_B2BBalance_Before = await b2bToken.balanceOf(frontEnd_2)
      const frontEnd_3_B2BBalance_Before = await b2bToken.balanceOf(frontEnd_3)

      assert.equal(frontEnd_1_B2BBalance_Before, '0')
      assert.equal(frontEnd_2_B2BBalance_Before, '0')
      assert.equal(frontEnd_3_B2BBalance_Before, '0')

      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

      // console.log(`B2BSupplyCap before: ${await communityIssuance.B2BSupplyCap()}`)
      // console.log(`totalB2BIssued before: ${await communityIssuance.totalB2BIssued()}`)
      // console.log(`B2B balance of CI before: ${await b2bToken.balanceOf(communityIssuance.address)}`)
      ...
      // console.log(`B2BSupplyCap after: ${await communityIssuance.B2BSupplyCap()}`)
      // console.log(`totalB2BIssued after: ${await communityIssuance.totalB2BIssued()}`)
      // console.log(`B2B balance of CI after: ${await b2bToken.balanceOf(communityIssuance.address)}`)

      // Get F1, F2, F3 B2B balances after, and confirm they have increased
      const frontEnd_1_B2BBalance_After = await b2bToken.balanceOf(frontEnd_1)
      const frontEnd_2_B2BBalance_After = await b2bToken.balanceOf(frontEnd_2)
      const frontEnd_3_B2BBalance_After = await b2bToken.balanceOf(frontEnd_3)

      assert.isTrue(frontEnd_1_B2BBalance_After.gt(frontEnd_1_B2BBalance_Before))
      assert.isTrue(frontEnd_2_B2BBalance_After.gt(frontEnd_2_B2BBalance_Before))
      assert.isTrue(frontEnd_3_B2BBalance_After.gt(frontEnd_3_B2BBalance_Before))
    })
    ...
    it("provideToSP(), new deposit: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers BUSDC to A, B
      await busdcToken.transfer(A, dec(100, 18), { from: whale })
      await busdcToken.transfer(B, dec(200, 18), { from: whale })

      // C, D open troves
      await openTrove({ extraBUSDCAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      // --- TEST ---

      // get current ETH balances
      const A_ETHBalance_Before = await contracts.backedToken.balanceOf(A)
      const B_ETHBalance_Before = await contracts.backedToken.balanceOf(B)
      const C_ETHBalance_Before = await contracts.backedToken.balanceOf(C)
      const D_ETHBalance_Before = await contracts.backedToken.balanceOf(D)
      ...
      const A_ETHBalance_After = await contracts.backedToken.balanceOf(A)
      const B_ETHBalance_After = await contracts.backedToken.balanceOf(B)
      const C_ETHBalance_After = await contracts.backedToken.balanceOf(C)
      const D_ETHBalance_After = await contracts.backedToken.balanceOf(D)
    ...
    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers BUSDC to A, B
      await busdcToken.transfer(A, dec(1000, 18), { from: whale })
      await busdcToken.transfer(B, dec(1000, 18), { from: whale })

      // C, D open troves
      await openTrove({ extraBUSDCAmount: toBN(dec(4000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), topup: triggers B2B reward event - increases the sum G", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Expect G has increased from the B2B reward event triggered by B's topup
      assert.isTrue(G_After.gt(G_Before))
    })
    ...
    it("provideToSP(), topup from different front end: doesn't change the front end tag", async () => {
      ...
      await busdcToken.transfer(D, dec(100, 18), { from: whale })
      await busdcToken.transfer(E, dec(200, 18), { from: whale })

      // A, B, C open troves
      await openTrove({ extraBUSDCAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraBUSDCAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraBUSDCAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("provideToSP(), topup: depositor receives B2B rewards", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Get A, B, C B2B balance before
      const A_B2BBalance_Before = await b2bToken.balanceOf(A)
      const B_B2BBalance_Before = await b2bToken.balanceOf(B)
      const C_B2BBalance_Before = await b2bToken.balanceOf(C)
      ...
      // Get B2B balance after
      const A_B2BBalance_After = await b2bToken.balanceOf(A)
      const B_B2BBalance_After = await b2bToken.balanceOf(B)
      const C_B2BBalance_After = await b2bToken.balanceOf(C)

      // Check B2B Balance of A, B, C has increased
      assert.isTrue(A_B2BBalance_After.gt(A_B2BBalance_Before))
      assert.isTrue(B_B2BBalance_After.gt(B_B2BBalance_Before))
      assert.isTrue(C_B2BBalance_After.gt(C_B2BBalance_Before))
    })

    it("provideToSP(), topup: tagged front end receives B2B rewards", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Get front ends' B2B balance before
      const F1_B2BBalance_Before = await b2bToken.balanceOf(frontEnd_1)
      const F2_B2BBalance_Before = await b2bToken.balanceOf(frontEnd_2)
      const F3_B2BBalance_Before = await b2bToken.balanceOf(frontEnd_3)
      ...
      // Get front ends' B2B balance after
      const F1_B2BBalance_After = await b2bToken.balanceOf(A)
      const F2_B2BBalance_After = await b2bToken.balanceOf(B)
      const F3_B2BBalance_After = await b2bToken.balanceOf(C)

      // Check B2B Balance of front ends has increased
      assert.isTrue(F1_B2BBalance_After.gt(F1_B2BBalance_Before))
      assert.isTrue(F2_B2BBalance_After.gt(F2_B2BBalance_Before))
      assert.isTrue(F3_B2BBalance_After.gt(F3_B2BBalance_Before))
    })
    ...
    it("provideToSP(), topup: tagged front end's stake increases", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraBUSDCAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraBUSDCAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraBUSDCAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("provideToSP(), topup: tagged front end's snapshots update", async () => {
      ...
      // A, B, C, open troves
      await openTrove({ extraBUSDCAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraBUSDCAmount: toBN(dec(400, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraBUSDCAmount: toBN(dec(600, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      await stabilityPool.provideToSP(await busdcToken.balanceOf(D), ZERO_ADDRESS, { from: D })
      ...
      // between topups, because some block.timestamp time passes (and B2B is issued) between ops
      const G1 = await stabilityPool.epochToScaleToG(currentScale, currentEpoch)
      await stabilityPool.provideToSP(deposit_A, frontEnd_1, { from: A })
    ...
    it("provideToSP(): reverts when amount is zero", async () => {
      ...
      // Whale transfers BUSDC to C, D
      await busdcToken.transfer(C, dec(100, 18), { from: whale })
      await busdcToken.transfer(D, dec(100, 18), { from: whale })
    ...
    it("provideToSP(): reverts if user is a registered front end", async () => {
      // C, D, E, F open troves
      await openTrove({ extraBUSDCAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraBUSDCAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraBUSDCAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("withdrawFromSP(): reverts when user has no active deposit", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraBUSDCAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    it("withdrawFromSP(): partial retrieval - retrieves correct BUSDC amount and the entire ETH Gain, and updates deposit", async () => {
      // --- SETUP ---
      // Whale deposits 185000 BUSDC in StabilityPool
      await openTrove({ extraBUSDCAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Alice makes deposit #1: 15000 BUSDC
      await openTrove({ extraBUSDCAmount: toBN(dec(15000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await stabilityPool.provideToSP(dec(15000, 18), frontEnd_1, { from: alice })

      // price drops: defaulters' Troves fall below MCR, alice and whale Trove remain active
      await priceFeed.setPrice(dec(105, 18));

      // 2 users with Trove with 170 BUSDC drawn are closed
      const liquidationTX_1 = await troveManager.liquidate(defaulter_1, { from: owner })  // 170 BUSDC closed
      const liquidationTX_2 = await troveManager.liquidate(defaulter_2, { from: owner }) // 170 BUSDC closed

      const [liquidatedDebt_1] = await th.getEmittedLiquidationValues(liquidationTX_1)
      const [liquidatedDebt_2] = await th.getEmittedLiquidationValues(liquidationTX_2)

      // Alice BUSDCLoss is ((15000/200000) * liquidatedDebt), for each liquidation
      const expectedBUSDCLoss_A = (liquidatedDebt_1.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))
        .add(liquidatedDebt_2.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))

      const expectedCompoundedBUSDCDeposit_A = toBN(dec(15000, 18)).sub(expectedBUSDCLoss_A)
      const compoundedBUSDCDeposit_A = await stabilityPool.getCompoundedBUSDCDeposit(alice)

      assert.isAtMost(th.getDifference(expectedCompoundedBUSDCDeposit_A, compoundedBUSDCDeposit_A), 100000)

      // Alice retrieves part of her entitled BUSDC: 9000 BUSDC
      await stabilityPool.withdrawFromSP(dec(9000, 18), { from: alice })

      const expectedNewDeposit_A = (compoundedBUSDCDeposit_A.sub(toBN(dec(9000, 18))))
    ...
    it("withdrawFromSP(): partial retrieval - leaves the correct amount of BUSDC in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 BUSDC in StabilityPool
      await openTrove({ extraBUSDCAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
      ...
      const SP_BUSDC_Before = await stabilityPool.getTotalBUSDCDeposits()
      assert.equal(SP_BUSDC_Before, dec(200000, 18))
      ...
      Expect BUSDC in SP = (200000 - liquidatedDebt_1 - liquidatedDebt_2 - 9000) */
      const expectedSPBUSDC = toBN(dec(200000, 18))
        .sub(toBN(liquidatedDebt_1))
        .sub(toBN(liquidatedDebt_2))
        .sub(toBN(dec(9000, 18)))

      const SP_BUSDC_After = (await stabilityPool.getTotalBUSDCDeposits()).toString()

      th.assertIsApproximatelyEqual(SP_BUSDC_After, expectedSPBUSDC)
    })

    it("withdrawFromSP(): full retrieval - leaves the correct amount of BUSDC in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 BUSDC in StabilityPool
      ...
      const BUSDCinSPBefore = await stabilityPool.getTotalBUSDCDeposits()

      // Alice retrieves all of her entitled BUSDC:
      await stabilityPool.withdrawFromSP(dec(15000, 18), { from: alice })

      const expectedBUSDCinSPAfter = BUSDCinSPBefore.sub(compoundedBUSDCDeposit_A)

      const BUSDCinSPAfter = await stabilityPool.getTotalBUSDCDeposits()
      assert.isAtMost(th.getDifference(expectedBUSDCinSPAfter, BUSDCinSPAfter), 100000)
    })
    ...
    it("withdrawFromSP(): it correctly updates the user's BUSDC and ETH snapshots of entitled reward per unit staked", async () => {
      // --- SETUP ---
      // Whale deposits 185000 BUSDC in StabilityPool
      await openTrove({ extraBUSDCAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
    ...
    it("withdrawFromSP(): decreases StabilityPool ETH", async () => {
      ...
      const liquidationTx_1 = await troveManager.liquidate(defaulter_1, { from: owner })  // 180 BUSDC closed
    ...
    it("withdrawFromSP(): All depositors are able to withdraw from the SP to their account", async () => {
      ...
      for (account of depositors) {
        await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: account } })
      ...
      const totalDeposits = (await stabilityPool.getTotalBUSDCDeposits()).toString()

      assert.isAtMost(th.getDifference(totalDeposits, '0'), 100000)
    })

    it("withdrawFromSP(): increases depositor's BUSDC token balance by the expected amount", async () => {
      // Whale opens trove
      await openTrove({ extraBUSDCAmount: toBN(dec(100000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      // 1 defaulter opens trove
      await contracts.backedToken.approve(borrowerOperations.address, dec(100, 'ether'), { from: defaulter_1 })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveBUSDCAmount(dec(10000, 18)), defaulter_1, defaulter_1, dec(100, 'ether'), { from: defaulter_1 })
      ...
      const aliceBalBefore = await busdcToken.balanceOf(alice)
      const bobBalBefore = await busdcToken.balanceOf(bob)

      /* From an offset of 10000 BUSDC, each depositor receives
      BUSDCLoss = 1666.6666666666666666 BUSDC

      and thus with a deposit of 10000 BUSDC, each should withdraw 8333.3333333333333333 BUSDC (in practice, slightly less due to rounding error)
      */

      // Price bounces back to $200 per ETH
      await priceFeed.setPrice(dec(200, 18))

      // Bob issues a further 5000 BUSDC from his trove
      await borrowerOperations.withdrawBUSDC(th._100pct, dec(5000, 18), bob, bob, { from: bob })

      // Expect Alice's BUSDC balance increase be very close to 8333.3333333333333333 BUSDC
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice })
      const aliceBalance = (await busdcToken.balanceOf(alice))

      assert.isAtMost(th.getDifference(aliceBalance.sub(aliceBalBefore), '8333333333333333333333'), 100000)

      // expect Bob's BUSDC balance increase to be very close to  13333.33333333333333333 BUSDC
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob })
      const bobBalance = (await busdcToken.balanceOf(bob))
    ...
    it("withdrawFromSP(): doesn't impact other users Stability deposits or ETH gains", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
      ...
      const A_ETHBalBefore = toBN(await contracts.backedToken.balanceOf(A))
      const A_B2BBalBefore = await b2bToken.balanceOf(A)

      // Check Alice has gains to withdraw
      const A_pendingETHGain = await stabilityPool.getDepositorETHGain(A)
      const A_pendingB2BGain = await stabilityPool.getDepositorB2BGain(A)
      assert.isTrue(A_pendingETHGain.gt(toBN('0')))
      assert.isTrue(A_pendingB2BGain.gt(toBN('0')))

      // Check withdrawal of 0 succeeds
      const tx = await stabilityPool.withdrawFromSP(0, { from: A, gasPrice: GAS_PRICE })
      assert.isTrue(tx.receipt.status)

      const A_ETHBalAfter = toBN(await contracts.backedToken.balanceOf(A))

      const A_B2BBalAfter = await b2bToken.balanceOf(A)
      const A_B2BBalDiff = A_B2BBalAfter.sub(A_B2BBalBefore)

      // Check A's ETH and B2B balances have increased correctly
      assert.isTrue(A_ETHBalAfter.sub(A_ETHBalBefore).eq(A_pendingETHGain))
      assert.isAtMost(th.getDifference(A_B2BBalDiff, A_pendingB2BGain), 1000)
    })

    it("withdrawFromSP(): withdrawing 0 BUSDC doesn't alter the caller's deposit or the total BUSDC in the Stability Pool", async () => {
      // --- SETUP ---
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Bob withdraws 0 BUSDC from the Stability Pool
      await stabilityPool.withdrawFromSP(0, { from: bob })

      // check Bob's deposit and total BUSDC in Stability Pool has not changed
      const bob_Deposit_After = (await stabilityPool.getCompoundedBUSDCDeposit(bob)).toString()
      const BUSDCinSP_After = (await stabilityPool.getTotalBUSDCDeposits()).toString()

      assert.equal(bob_Deposit_Before, bob_Deposit_After)
      assert.equal(BUSDCinSP_Before, BUSDCinSP_After)
    })
    ...
    it("withdrawFromSP(): withdrawing 0 ETH Gain does not alter the caller's ETH balance, their trove collateral, or the ETH  in the Stability Pool", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      ...
      const dennis_ETHBalance_Before = (contracts.backedToken.balanceOf(dennis)).toString()
      ...
      const dennis_ETHBalance_After = (contracts.backedToken.balanceOf(dennis)).toString()
    ...
    it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
      ...
      const alice_BUSDC_Balance_Before = await busdcToken.balanceOf(alice)
      const bob_BUSDC_Balance_Before = await busdcToken.balanceOf(bob)

      const alice_Deposit_Before = await stabilityPool.getCompoundedBUSDCDeposit(alice)
      const bob_Deposit_Before = await stabilityPool.getCompoundedBUSDCDeposit(bob)

      const BUSDCinSP_Before = await stabilityPool.getTotalBUSDCDeposits()
      ...
      // Check Bob's BUSDC balance has risen by only the value of his compounded deposit
      const bob_expectedBUSDCBalance = (bob_BUSDC_Balance_Before.add(bob_Deposit_Before)).toString()
      const bob_BUSDC_Balance_After = (await busdcToken.balanceOf(bob)).toString()
      assert.equal(bob_BUSDC_Balance_After, bob_expectedBUSDCBalance)

      // Alice attempts to withdraws 2309842309.000000000000000000 BUSDC from the Stability Pool
      await stabilityPool.withdrawFromSP('2309842309000000000000000000', { from: alice })

      // Check Alice's BUSDC balance has risen by only the value of her compounded deposit
      const alice_expectedBUSDCBalance = (alice_BUSDC_Balance_Before.add(alice_Deposit_Before)).toString()
      const alice_BUSDC_Balance_After = (await busdcToken.balanceOf(alice)).toString()
      assert.equal(alice_BUSDC_Balance_After, alice_expectedBUSDCBalance)

      // Check BUSDC in Stability Pool has been reduced by only Alice's compounded deposit and Bob's compounded deposit
      const expectedBUSDCinSP = (BUSDCinSP_Before.sub(alice_Deposit_Before).sub(bob_Deposit_Before)).toString()
      const BUSDCinSP_After = (await stabilityPool.getTotalBUSDCDeposits()).toString()
      assert.equal(BUSDCinSP_After, expectedBUSDCinSP)
    })

    it("withdrawFromSP(): Request to withdraw 2^256-1 BUSDC only withdraws the caller's compounded deposit", async () => {
      // --- SETUP ---
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Bob attempts to withdraws maxBytes32 BUSDC from the Stability Pool
      ...
      // Check BUSDC in Stability Pool has been reduced by only  Bob's compounded deposit
      const expectedBUSDCinSP = (BUSDCinSP_Before.sub(bob_Deposit_Before)).toString()
      const BUSDCinSP_After = (await stabilityPool.getTotalBUSDCDeposits()).toString()
      assert.equal(BUSDCinSP_After, expectedBUSDCinSP)
    })
    ...
    it("withdrawFromSP(): caller can withdraw full deposit and ETH gain during Recovery Mode", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      // Price halves
      await priceFeed.setPrice(dec(200, 18))

      // A, B, C open troves and make Stability Pool deposits
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
      await openTrove({ extraBUSDCAmount: toBN(dec(20000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: bob } })
      await openTrove({ extraBUSDCAmount: toBN(dec(30000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: carol } })

      await contracts.backedToken.approve(borrowerOperations.address, dec(100, 'ether'), { from: defaulter_1 })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveBUSDCAmount(dec(10000, 18)), defaulter_1, defaulter_1, dec(100, 'ether'), { from: defaulter_1 })

      // A, B, C provides 10000, 5000, 3000 BUSDC to SP
      ...
      const alice_BUSDC_Balance_Before = await busdcToken.balanceOf(alice)
      const bob_BUSDC_Balance_Before = await busdcToken.balanceOf(bob)
      const carol_BUSDC_Balance_Before = await busdcToken.balanceOf(carol)

      const alice_ETH_Balance_Before = web3.utils.toBN(await contracts.backedToken.balanceOf(alice))
      const bob_ETH_Balance_Before = web3.utils.toBN(await contracts.backedToken.balanceOf(bob))
      const carol_ETH_Balance_Before = web3.utils.toBN(await contracts.backedToken.balanceOf(carol))

      const alice_Deposit_Before = await stabilityPool.getCompoundedBUSDCDeposit(alice)
      const bob_Deposit_Before = await stabilityPool.getCompoundedBUSDCDeposit(bob)
      const carol_Deposit_Before = await stabilityPool.getCompoundedBUSDCDeposit(carol)
      ...
      // Check BUSDC balances of A, B, C have risen by the value of their compounded deposits, respectively
      const alice_expectedBUSDCBalance = (alice_BUSDC_Balance_Before.add(alice_Deposit_Before)).toString()

      const bob_expectedBUSDCBalance = (bob_BUSDC_Balance_Before.add(bob_Deposit_Before)).toString()
      const carol_expectedBUSDCBalance = (carol_BUSDC_Balance_Before.add(carol_Deposit_Before)).toString()

      const alice_BUSDC_Balance_After = (await busdcToken.balanceOf(alice)).toString()

      const bob_BUSDC_Balance_After = (await busdcToken.balanceOf(bob)).toString()
      const carol_BUSDC_Balance_After = (await busdcToken.balanceOf(carol)).toString()



      assert.equal(alice_BUSDC_Balance_After, alice_expectedBUSDCBalance)
      assert.equal(bob_BUSDC_Balance_After, bob_expectedBUSDCBalance)
      assert.equal(carol_BUSDC_Balance_After, carol_expectedBUSDCBalance)
      ...
      const alice_ETHBalance_After = (await contracts.backedToken.balanceOf(alice)).toString()
      const bob_ETHBalance_After = (await contracts.backedToken.balanceOf(bob)).toString()
      const carol_ETHBalance_After = (await contracts.backedToken.balanceOf(carol)).toString()
      ...
      // Check BUSDC in Stability Pool has been reduced by A, B and C's compounded deposit
      const expectedBUSDCinSP = (BUSDCinSP_Before
    ...
    it("getDepositorETHGain(): depositor does not earn further ETH gains from liquidations while their compounded deposit == 0: ", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2 } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_3 } })

      // A, B, provide 10000, 5000 BUSDC to SP
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: alice })
      ...
      const BUSDCinSP = (await stabilityPool.getTotalBUSDCDeposits()).toString()
      assert.equal(BUSDCinSP, '0')

      // Check Stability deposits have been fully cancelled with debt, and are now all zero
      const alice_Deposit = (await stabilityPool.getCompoundedBUSDCDeposit(alice)).toString()
      const bob_Deposit = (await stabilityPool.getCompoundedBUSDCDeposit(bob)).toString()
      ...
      // Whale deposits 10000 BUSDC to Stability Pool
    ...
    // --- B2B functionality ---
    it("withdrawFromSP(): triggers B2B reward event - increases the sum G", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      // A, B, C open troves
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraBUSDCAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraBUSDCAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdrawFromSP(), partial withdrawal: depositor receives B2B rewards", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end receives B2B rewards", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's stake decreases", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraBUSDCAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraBUSDCAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraBUSDCAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's snapshots update", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(60000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })

      // D opens trove
      await openTrove({ extraBUSDCAmount: toBN(dec(100000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("withdrawFromSP(), full withdrawal: removes deposit's front end tag", async () => {
      ...
      await busdcToken.transfer(A, dec(10000, 18), { from: whale })
      await busdcToken.transfer(B, dec(20000, 18), { from: whale })
    ...
    it("withdrawFromSP(), full withdrawal: zero's depositor's snapshots", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(20000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: E } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })

      // Fast-forward time and make a second deposit, to trigger B2B reward and make G > 0
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })
      ...
      // C, D open troves
      await openTrove({ extraBUSDCAmount: toBN(dec(30000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(40000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: D } })
    ...
    it("withdrawFromSP(), full withdrawal that reduces front end stake to 0: zero’s the front end’s snapshots", async () => {
      ...
      // E opens trove and makes a deposit
      await openTrove({ extraBUSDCAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })
      ...
      // A, B open troves
      await openTrove({ extraBUSDCAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraBUSDCAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
    ...
    it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
      ...
      await openTrove({ extraBUSDCAmount: toBN(dec(10100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: A })

      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })

      //  SETUP: Execute a series of operations to trigger B2B and ETH rewards for depositor A
    ...
    it("withdrawETHGainToTrove(): Applies BUSDCLoss to user's deposit, and redirects ETH reward to user's Trove", async () => {
      // --- SETUP ---
      // Whale deposits 185000 BUSDC in StabilityPool
      await openTrove({ extraBUSDCAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
      ...
      const compoundedDeposit_A = await stabilityPool.getCompoundedBUSDCDeposit(alice)
      ...
      const expectedBUSDCLoss_A = liquidatedDebt.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18)))
      const expectedCompoundedDeposit_A = toBN(dec(15000, 18)).sub(expectedBUSDCLoss_A)
      ...
      // check Alice's BUSDCLoss has been applied to her deposit expectedCompoundedDeposit_A
    ...
    it("withdrawETHGainToTrove(): reverts if user has no trove", async () => {
     ...
     await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      ...
      // A transfers BUSDC to D
      await busdcToken.transfer(dennis, dec(10000, 18), { from: alice })
    ...
    it("withdrawETHGainToTrove(): triggers B2B reward event - increases the sum G", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawETHGainToTrove(), eligible deposit: depositor receives B2B rewards", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawETHGainToTrove(), eligible deposit: tagged front end receives B2B rewards", async () => {
      await openTrove({ extraBUSDCAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
     ...
     await priceFeed.setPrice(dec(105, 18))
      ...
      // Get front ends' B2B balance after
      const F1_B2BBalance_After = await b2bToken.balanceOf(frontEnd_1)
      const F2_B2BBalance_After = await b2bToken.balanceOf(frontEnd_2)
      const F3_B2BBalance_After = await b2bToken.balanceOf(frontEnd_3)
    ...
    it("withdrawETHGainToTrove(): reverts when depositor has no ETH gain", async () => {
      ...
      // fastforward time, and E makes a deposit, creating B2B rewards for all
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
      await openTrove({ extraBUSDCAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("registerFrontEnd(): reverts if address has a non-zero deposit already", async () => {
      // C, D, E open troves
      await openTrove({ extraBUSDCAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraBUSDCAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraBUSDCAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
